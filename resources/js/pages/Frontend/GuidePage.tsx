...
const UI: Record<string, { fr: string; ar: string }> = {
    ...
    ctaDesc:      { fr: 'Artisans vérifiés — contact WhatsApp direct — 0% commission', ar: 'حرفيون موثقون — تواصل واتساب مباشر — 0% عمولة' },
                    ...
                    <tbody>
                        {rows.map((row, i) => (
                            <tr key={i} className="border-b border-slate-100 dark:border-slate-800 even:bg-slate-50/50 dark:even:bg-slate-900/30">
                                {row.map((cell, j) => (
                                    <td key={j} className="px-4 py-2.5 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700"
...
export default function GuidePage({ article, relatedArticles, canonical }: Props) {
    ...
    const jsonLd = {
        ...
        publisher: { '@type': 'Organization', name: 'Jobly', url: typeof window !== 'undefined' ? window.location.origin : '' },
        url: canonical,
    };
            ...
            <div className="min-h-screen bg-white dark:bg-slate-900">
                <div className="max-w-3xl mx-auto px-4 py-10">
                    ...
                    <nav className="flex items-center gap-2 text-xs text-slate-400 mb-8">
                        <Link href="/" className="hover:text-orange-500 transition-colors">{tr('home')}</Link>
                        <ChevronRight className="h-3 w-3 rtl:rotate-180" />
                        <Link href="/guides" className="hover:text-orange-500 transition-colors">{tr('guides')}</Link>
                        <ChevronRight className="h-3 w-3 rtl:rotate-180" />
                    ...
                    {relArray.length > 0 && (
                        <div className="mt-12">
                            <h2 className="text-base font-bold text-slate-900 dark:text-white mb-4">{tr('related')}</h2>
