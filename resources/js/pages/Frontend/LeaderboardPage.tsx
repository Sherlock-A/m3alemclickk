...
function FaqItem({ faq, index }: { faq: Faq; index: number }) {
    ...
    return (
        <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
            <button
                onClick={() => setOpen((o) => !o)}
                className="w-full flex items-center justify-between gap-3 px-5 py-4 text-left bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                aria-expanded={open}
            >
...
export default function LeaderboardPage({ city, category, catSlug, pros, faqs, seoTitle, seoDesc, faqSchema, canonical }: Props) {
    const whatsappUrl = (pro: Pro) =>
        `/api/whatsapp/${pro.id}`;
            ...
            <Head>
                ...
                <script type="application/ld+json">{JSON.stringify({
                    ...
                    itemListElement: [
                        ...
                        { '@type': 'ListItem', position: 2, name: `${category}s à ${city}`, item: `/professionnels/${city.toLowerCase()}/${catSlug}` },
                        { '@type': 'ListItem', position: 3, name: `Top 10 ${category}s à ${city}` },
                    ],
                })}</script>
            </Head>
                ...
                <div className="bg-gradient-to-br from-orange-500 to-orange-600 text-white py-14 px-4">
                    <div className="max-w-3xl mx-auto text-center">
                        <div className="flex items-center justify-center gap-2 mb-4">
                            ...
                            <span className="text-sm font-semibold bg-white/20 rounded-full px-3 py-1">Classement Jobly</span>
                        </div>
                        <h1 className="text-3xl md:text-4xl font-extrabold mb-3 leading-tight">
                            Top 10 {category}s<br />à {city}
                        </h1>
                        <p className="text-orange-100 text-base max-w-xl mx-auto">
                            Les meilleurs {category.toLowerCase()}s vérifiés, classés par note clients. Contactez-les directement sur WhatsApp.
                        </p>
                ...
                <div className="max-w-3xl mx-auto px-4 py-10">
                    ...
                    {pros.length === 0 ? (
                        <div className="text-center py-16 text-slate-500 dark:text-slate-400">
                            ...
                            <p className="font-semibold">Aucun {category.toLowerCase()} classé à {city} pour l'instant.</p>
                            <p className="text-sm mt-2">Revenez bientôt — le classement se met à jour régulièrement.</p>
                            <Link
                            ...
                            >
                                Voir tous les {category.toLowerCase()}s
                            </Link>
                        </div>
                    ) : (
                                            ...
                                            <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                                                ...
                                                {pro.completed_missions > 0 && (
                                                    <span className="text-xs text-slate-400 dark:text-slate-500">
                                                        {pro.completed_missions} mission{pro.completed_missions > 1 ? 's' : ''}
                                                    </span>
                                                )}
                                            </div>
                                        </div>
                    ...
                    <div className="mt-8 text-center">
                        ...
                        >
                            Voir tous les {category.toLowerCase()}s à {city}
                    ...
                    <p className="mt-10 text-center text-xs text-slate-400 dark:text-slate-600">
                        Classement mis à jour toutes les heures · Basé sur les avis vérifiés Jobly · {pros.length} professionnel{pros.length > 1 ? 's' : ''} classé{pros.length > 1 ? 's' : ''}
                    </p>
                </div>
            </div>
        </Layout>
